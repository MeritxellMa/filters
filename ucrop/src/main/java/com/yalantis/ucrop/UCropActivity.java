...
public class UCropActivity extends AppCompatActivity {
    ...
    // Constants
    private static final String INTERNAL_DIRECTORY_STORAGE_IMAGES_NAME = "Yumehub";
...
/*    @Override
    ...
    private void setImageData(@NonNull Intent intent) {
        ...
        requestCodeView = extras.getInt("requestCodeView");
...
//            mWrapperStateAspectRatio.setVisibility(View.GONE);
    ...
    private void setupViews(@NonNull Intent intent) {
        ...
        mToolbarTextColor = intent.getIntExtra(UCrop.Options.EXTRA_UCROP_TITLE_COLOR_TOOLBAR, ContextCompat.getColor(this, R.color.ucrop_color_title));
...
/*    private void setupScaleWidget() {
    ...
    private void cropAndSaveImage() {
        ...
        try {
            ...
            if (croppedBitmap != null) {
                ...
                setResultUri(new SaveCroppedImage().execute(croppedBitmap).get(), mGestureCropImageView.getTargetAspectRatio(), requestCodeView);
    ...
    private void setResultUri(Uri uri, float resultAspectRatio, int requestCodeView) {
        setResult(RESULT_OK, new Intent()
                .putExtra(UCrop.EXTRA_OUTPUT_URI, uri)
                .putExtra(UCrop.EXTRA_OUTPUT_CROP_ASPECT_RATIO, resultAspectRatio)
                .putExtra("requestCodeView", requestCodeView));
    }
    ...
    private File getStorageDir() {
        ...
        if (haveSd) {
            storageDir = new File(Environment.getExternalStoragePublicDirectory(
                    Environment.DIRECTORY_PICTURES), INTERNAL_DIRECTORY_STORAGE_IMAGES_NAME);
        } else {
        ...
        if (storageDir != null) {
            if (!storageDir.mkdirs()) {
                if (!storageDir.exists()) {
                    Log.e(TAG, "storageDir - failed to create directory");
                    return null;
                }
            }
        }
    ...
    private File saveBitmapInFile(File storageDir, Bitmap inImage) {

        //create name for picture
        Date now = new Date();
        SimpleDateFormat sdf = new SimpleDateFormat("_yyyyMMdd_hhmmss");
        String fileName = "YMH" + sdf.format(now);
        ...
        try {
            ...
            inImage.compress(DEFAULT_COMPRESS_FORMAT, DEFAULT_COMPRESS_QUALITY, out);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            try {
        ...
        Log.i("path", imageFile.getAbsolutePath());
        return imageFile;
    }

    private void insertPicInContentResolver(File imageFile) {
        //set values
        ...
        values.put(MediaStore.Images.Media.DATE_TAKEN, imageFile.lastModified());
        values.put(MediaStore.Images.Media.MIME_TYPE, "image/jpeg");
        values.put(MediaStore.Images.Media.ORIENTATION, getOrientationOfPic(imageFile));
        values.put(MediaStore.Images.Media.DATA, imageFile.getAbsolutePath());
        getContentResolver().insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values);

    }

    private String getOrientationOfPic(File imageFile) {
    ...
    private class SaveCroppedImage extends AsyncTask<Bitmap, Void, Uri> {
        @Override
        protected Uri doInBackground(Bitmap... params) {
            ...
            if (storageDir != null) {
                ...
                File imageFile = saveBitmapInFile(storageDir, params[0]);

                if (imageFile != null) {
                    //let the gallery reconize the picture
                    insertPicInContentResolver(imageFile);
                }
                ...
                Log.e(TAG, "storageDir - there's no storageDir directory");
                return null;
